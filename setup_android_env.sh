...
update_gradle_wrapper_properties() {
  ...
  if [[ "${USE_LOCAL_GRADLE_WRAPPER:-0}" != "1" ]]; then
    log "Keeping the network Gradle Wrapper distribution URL"
    return 0
  fi
...
replace_aapt2() {
  if [[ "$(uname -m)" != "aarch64" && "$(uname -m)" != "arm64" ]]; then
    log "Non-ARM64 host detected; keeping the platform AAPT2 binary"
    return 0
  fi

  local bundled_aapt2="$SCRIPT_DIR/tools/aapt2/aapt2-arm64-v8a"
  local aapt2_url="https://github.com/ReVanced/aapt2/releases/download/v1.0.0/aapt2-arm64-v8a"
  local expected_sha256="e5b5ff7f0d4f6ecd7fa5d05d77fed3f09f6f1bf80f078b8aada82bc578848561"

  # The binary is downloaded on demand so the repository stays source-only.
  if [[ ! -f "$bundled_aapt2" ]]; then
    log "ARM64 aapt2 not found; downloading a verified copy"
    mkdir -p "$(dirname "$bundled_aapt2")"
    download_file "$aapt2_url" "$bundled_aapt2"
    chmod +x "$bundled_aapt2"
  fi
